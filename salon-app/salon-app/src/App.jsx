...
// ─── CONFIG — change these ─────────────────────────────────────────────────
const ADMIN_EMAIL    = "admin@salon.com";   // your email
const ADMIN_PASSWORD = "admin123";           // your password
const YOUR_UPI_ID    = "yoursalon@upi";     // your FamPay/UPI ID
...
export default function App() {
  ...
  useEffect(() => {
    if (screen !== "login") loadBookings();
  }, [screen]);

  // Real-time updates — bookings refresh automatically when anyone makes one
  useEffect(() => {
  ...
  async function doBooking() {
    ...
    if (error) { setError("Booking failed. Try again."); setLoading(false); return; }
  ...
  // ── STYLES ──────────────────────────────────────────────────────────────
  const G="1D9E75", GL="#E1F5EE", GD="#0F6E56", GHEX=`#${G}`;
  const css = {
    ...
    top:   { background:"var(--color-background-primary)", borderBottom:"0.5px solid var(--color-border-tertiary)", padding:"0 1rem", height:52, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:10 },
    btn:   { padding:"11px 20px", background:GHEX, color:"#fff", border:"none", borderRadius:8, fontSize:14, fontWeight:500, cursor:"pointer", width:"100%" },
    ...
    inp:   { width:"100%", padding:"10px 12px", border:"0.5px solid var(--color-border-tertiary)", borderRadius:8, background:"var(--color-background-primary)", color:"var(--color-text-primary)", fontSize:14, marginTop:6, marginBottom:12, display:"block" },
  ...
  const Bar = () => (
    <div style={css.top}>
      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        ...
        <span style={{ ...css.badge("x"), background:screen==="admin"?GL:"#E6F1FB", color:screen==="admin"?GD:"#185FA5", fontSize:11, padding:"3px 10px", borderRadius:20, fontWeight:500 }}>
      ...
      <div style={{ maxWidth:400, margin:"0 auto", padding:"3rem 1rem" }}>
        <div style={css.card}>
          ...
          <div style={{ display:"flex", gap:6, background:"var(--color-background-secondary)", borderRadius:8, padding:4, marginBottom:"1.5rem" }}>
            {["customer","admin"].map(t=>(
              <button key={t} onClick={()=>{ setTab(t); setError(""); }}
                style={{ flex:1, padding:"8px 0", border:tab===t?"0.5px solid var(--color-border-tertiary)":"none", background:tab===t?"var(--color-background-primary)":"transparent", borderRadius:6, fontSize:13, fontWeight:tab===t?500:400, color:tab===t?"var(--color-text-primary)":"var(--color-text-secondary)", cursor:"pointer" }}>
          ...
          {tab==="customer" ? <>
            ...
            <input style={css.inp} placeholder="Priya Sharma" value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))} />
            <label style={css.lbl}>Phone number</label>
            <input style={css.inp} placeholder="9876543210" value={form.phone} onChange={e=>setForm(p=>({...p,phone:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&loginCustomer()} />
            <button style={css.btn} onClick={loginCustomer}>Book an appointment →</button>
          </> : <>
            <label style={css.lbl}>Email</label>
            <input style={css.inp} placeholder="admin@salon.com" value={form.email} onChange={e=>setForm(p=>({...p,email:e.target.value}))} />
            <label style={css.lbl}>Password</label>
            <input style={css.inp} type="password" placeholder="••••••••" value={form.password} onChange={e=>setForm(p=>({...p,password:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&loginAdmin()} />
  ...
  if (screen==="customer") return (
    <div style={css.page}>
      <Bar />
      <div style={css.wrap}>
        {loading && <div style={{ textAlign:"center", padding:"2rem", fontSize:13, color:"var(--color-text-secondary)" }}>Loading...</div>}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:10, marginBottom:"1.5rem" }}>
          {SERVICES.map(svc=>(
            <div key={svc.id} onClick={()=>!onLeave&&setSelSvc(selSvc?.id===svc.id?null:svc)}
              style={{ background:selSvc?.id===svc.id?GL:"var(--color-background-primary)", border:selSvc?.id===svc.id?`1.5px solid ${GHEX}`:"0.5px solid var(--color-border-tertiary)", borderRadius:12, padding:"1rem", cursor:onLeave?"not-allowed":"pointer", opacity:onLeave?0.5:1, transition:"all 0.15s" }}>
              ...
              <div style={{ fontSize:13, color:GHEX, fontWeight:500, marginTop:4 }}>₹{svc.price}</div>
        ...
        {selSvc && !onLeave && (
          <div style={css.card}>
            <p style={{ fontSize:15, fontWeight:500, marginBottom:"1rem" }}>
              Booking: <span style={{ color:GHEX }}>{selSvc.name}</span> — ₹{selSvc.price}
            </p>
            <label style={css.lbl}>Select date</label>
            <input type="date" style={css.inp} min={today} value={selDate} onChange={e=>{ setSelDate(e.target.value); setSelSlot(null); setPayStep(false); }} />
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginTop:8, marginBottom:"1rem" }}>
              {SLOTS.map(slot=>{
                ...
                return (
                  <button key={slot} disabled={taken} onClick={()=>{ setSelSlot(slot); setPayStep(false); }}
                    style={{ padding:"7px 14px", borderRadius:8, fontSize:13, cursor:taken?"not-allowed":"pointer", border:selSlot===slot?`1.5px solid ${GHEX}`:"0.5px solid var(--color-border-tertiary)", background:taken?"var(--color-background-secondary)":selSlot===slot?GHEX:"var(--color-background-primary)", color:taken?"var(--color-text-tertiary)":selSlot===slot?"#fff":"var(--color-text-primary)", textDecoration:taken?"line-through":"none" }}>
                    {slot}{taken?" ✗":""}
                  </button>
                );
              })}
            ...
            {selSlot && !payStep && (
              <>
                <div style={{ background:GL, border:"0.5px solid #9FE1CB", borderRadius:8, padding:"12px 16px", marginBottom:"1rem" }}>
                  <p style={{ fontSize:13, color:"#085041" }}><strong>{selSvc.name}</strong> · {selDate} · {selSlot}<br/>Total: <strong>₹{selSvc.price}</strong></p>
                </div>
              ...
              <div>
                ...
                <div style={{ background:"var(--color-background-secondary)", borderRadius:12, padding:"1.5rem", textAlign:"center", marginBottom:"1rem" }}>
                  <div style={{ width:130, height:130, background:"var(--color-background-primary)", margin:"0 auto 1rem", borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center", border:"0.5px solid var(--color-border-tertiary)" }}>
                    <div style={{ fontSize:11, color:"var(--color-text-secondary)", textAlign:"center", padding:8 }}>Add your UPI<br/>QR code here<br/>(put image in public/)</div>
                  </div>
                  <p style={{ fontSize:13, color:"var(--color-text-secondary)", marginBottom:8 }}>Or pay to UPI ID:</p>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
                    <code style={{ fontSize:14, fontWeight:500, color:"var(--color-text-primary)", background:"var(--color-background-primary)", padding:"6px 14px", borderRadius:6, border:"0.5px solid var(--color-border-tertiary)" }}>{YOUR_UPI_ID}</code>
                    <button style={css.btnSm} onClick={()=>{ navigator.clipboard?.writeText(YOUR_UPI_ID); setCopied(true); setTimeout(()=>setCopied(false),2000); }}>{copied?"Copied!":"Copy"}</button>
                  </div>
                  <p style={{ fontSize:11, color:"var(--color-text-tertiary)", marginTop:8 }}>Amount: ₹{selSvc.price}</p>
                </div>
                <div style={{ display:"flex", gap:8 }}>
                  <button style={{ ...css.btnSm, flex:1, padding:11 }} onClick={()=>setPayStep(false)}>← Back</button>
                  <button style={{ ...css.btn, flex:2 }} onClick={doBooking} disabled={loading}>{loading?"Saving...":"I've paid — Confirm →"}</button>
                </div>
                <p style={{ fontSize:11, color:"var(--color-text-tertiary)", textAlign:"center", marginTop:8 }}>Owner verifies payment and confirms your booking</p>
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ marginTop:"1.5rem" }}>
          ...
          {myBookings.length===0
            ? <p style={{ fontSize:13, color:"var(--color-text-secondary)" }}>No bookings yet. Pick a service above.</p>
            : myBookings.map(b=>(
              <div key={b.id} style={{ ...css.card, display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                <div>
                  ...
                  <div style={{ fontSize:12, color:b.paid?"#0F6E56":"#854F0B", marginTop:3 }}>{b.paid?"✓ Payment received":"⏳ Payment verification pending"}</div>
                  {b.status==="pending" && <button style={{ ...css.btnSm, marginTop:8 }} onClick={()=>cancelBk(b.id)}>Cancel</button>}
                </div>
    ...
    <div style={css.page}>
      ...
      <div style={css.wrap}>
        ...
        <div style={css.card}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:onLeave?"1rem":0 }}>
            <div>
              ...
              <div style={{ fontSize:12, color:"var(--color-text-secondary)", marginTop:2 }}>Customers see a notice and cannot book</div>
            </div>
            <label style={{ position:"relative", width:44, height:24, flexShrink:0, cursor:"pointer" }}>
              ...
              <span style={{ position:"absolute", top:0, left:0, right:0, bottom:0, background:onLeave?"#E24B4A":"var(--color-border-secondary)", borderRadius:24, transition:"0.2s", display:"block" }}>
          ...
          {onLeave && <input style={css.inp} placeholder="Leave message for customers..." value={leaveMsg} onChange={e=>setLeaveMsg(e.target.value)} />}
        </div>
        ...
        <div style={{ background:"var(--color-background-primary)", border:"0.5px solid var(--color-border-tertiary)", borderRadius:12, overflow:"hidden" }}>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              {["all","pending","confirmed","cancelled"].map(f=>(
                <button key={f} onClick={()=>setFilter(f)}
                  style={{ padding:"5px 10px", border:"0.5px solid var(--color-border-tertiary)", borderRadius:8, fontSize:12, background:filter===f?GHEX:"transparent", color:filter===f?"#fff":"var(--color-text-secondary)", cursor:"pointer" }}>
                  {f}
                </button>
              ))}
            </div>
          ...
          {loading
            ? <p style={{ padding:"1rem", fontSize:13, color:"var(--color-text-secondary)" }}>Loading bookings...</p>
            : filtered.length===0
            ...
            : filtered.map(b=>(
              <div key={b.id} style={{ padding:"14px 1.25rem", borderBottom:"0.5px solid var(--color-border-tertiary)", display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12 }}>
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:12, color:"var(--color-text-secondary)", marginTop:2 }}>📞 {b.phone} · {b.service} · {b.date} · {b.slot}</div>
                  <div style={{ fontSize:12, marginTop:2, color:b.paid?"#0F6E56":"#854F0B" }}>{b.paid?"✓ Paid · ₹"+b.price:"⏳ Not verified · ₹"+b.price}</div>
                </div>
                <div style={{ display:"flex", flexDirection:"column", gap:6, alignItems:"flex-end", flexShrink:0 }}>
                  <span style={css.badge(b.status)}>{b.status}</span>
                  {b.status==="pending" && (
                    <div style={{ display:"flex", gap:6 }}>
                      <button style={{ padding:"5px 10px", background:GHEX, color:"#fff", border:"none", borderRadius:6, fontSize:12, cursor:"pointer" }} onClick={()=>adminAction(b.id,"confirmed")}>Confirm</button>
                      <button style={{ ...css.btnSm, padding:"5px 10px" }} onClick={()=>adminAction(b.id,"cancelled")}>Reject</button>
                    </div>
                  )}
                  {!b.paid && b.status!=="cancelled" && (
                    <button style={{ padding:"4px 10px", background:GL, color:GD, border:"0.5px solid #9FE1CB", borderRadius:6, fontSize:12, cursor:"pointer" }} onClick={()=>markPaid(b.id)}>Mark paid</button>
                  )}
                </div>
              </div>
            ))
